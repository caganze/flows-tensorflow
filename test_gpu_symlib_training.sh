#!/bin/bash
# Test script for GPU compute node - trains actual particles using symlib format

echo "🧪 GPU SYMLIB TRAINING TEST"
echo "============================"
echo "Node: $(hostname)"
...
test_particle() {
    ...
    python train_tfp_flows.py \
        ...
        --generate-samples \
        > "test_logs/gpu_${size_type}_pid${PID}.log" 2>&1
    ...
    if [[ $TRAIN_EXIT -eq 0 ]]; then
        ...
        echo "   📋 ERROR LOG:"
        echo "   ============="
        if [[ -f "test_logs/gpu_${size_type}_pid${PID}.log" ]]; then
            tail -20 "test_logs/gpu_${size_type}_pid${PID}.log" | sed 's/^/   /'
        else
...
# Run tests
echo "🚀 Starting GPU training tests..."
echo "================================="
...
# Results
echo "🎉 GPU SYMLIB TRAINING TEST COMPLETE!"
echo "====================================="
