#!/usr/bin/env python3
"""
KDE-Informed Unconditional Continuous Flow Training Implementation

This script implements KDE-informed training for continuous normalizing flows (CNF) using Neural ODEs.
The approach combines KDE and continuous flow training through regularization in the loss function.

ARCHITECTURE: Unconditional Continuous Flow
...
- MSE loss between continuous flow and KDE log probabilities

Key features:
1. KDE density evaluation for continuous flow training batches
2. Combined loss function: L = L_NLL + λ * L_KDE
3. MSE between continuous flow and KDE log probabilities as regularization
4. Neural ODE-based continuous flow architecture
5. Toy dataset generation for testing
...
try:
    from cnf_flows_solution import CNFNormalizingFlow, CNFFlowTrainer, ConditionalCNFNormalizingFlow, ConditionalCNFFlowTrainer
except ImportError:
    print("Warning: Could not import CNF flow classes. Using simplified versions.")
    
    # Simplified fallback classes
    class NeuralODE(tf.keras.Model):
        """Simplified Neural ODE for continuous flows"""
    ...
    class CNFNormalizingFlow:
        """Simplified Continuous Normalizing Flow"""
        ...
        def forward_transform(self, x):
            ...
            return self.neural_ode(x, tf.constant(0.0))
        
        def inverse_transform(self, z):
            """Inverse transform: z -> x"""
            # Simple inverse pass (in practice, would use ODE solver)
            return self.neural_ode(z, tf.constant(0.0))
    ...
    class CNFFlowTrainer:
        ...
        def train(self, train_data, val_data=None, epochs=100, batch_size=512, validation_freq=10, verbose=True):
            """Train the flow"""
...
class KDEInformedCNFTrainer:
    """
    KDE-Informed Continuous Flow Trainer
    
    This trainer uses KDE density estimates to guide continuous flow training.
    The loss function combines negative log-likelihood with MSE between continuous flow and KDE
    log probabilities: L = L_NLL + λ * L_KDE
    """
    
    def __init__(self, flow: CNFNormalizingFlow, kde_model: gaussian_kde,
                 learning_rate: float = 1e-3, lambda_kde: float = 0.1,
                 weight_decay: float = 0.0, noise_std: float = 0.0):
        """
        Initialize KDE-informed continuous flow trainer
        
        Parameters:
        -----------
        flow : CNFNormalizingFlow
            The continuous flow model to train
    ...
    def compute_mse_log_probs(self, cnf_log_probs: tf.Tensor, kde_log_probs: tf.Tensor) -> tf.Tensor:
        """
        Compute MSE between continuous flow and KDE log probabilities
        
        MSE = E[(log P_CNF - log P_KDE)^2]
        
        This is more computationally efficient and effective than KL divergence
        for making the continuous flow's density match the KDE's density.
        
        Parameters:
        -----------
        cnf_log_probs : tf.Tensor
            Log probabilities from continuous flow
        kde_log_probs : tf.Tensor
    ...
    def compute_loss(self, x: tf.Tensor) -> Tuple[tf.Tensor, tf.Tensor, tf.Tensor]:
        """
        Compute combined loss: L = L_NLL + λ * L_KDE
        
        Where L_KDE is the MSE between continuous flow and KDE log probabilities:
        ...
        cnf_log_probs = self.flow.log_prob(x)
    ...
    @tf.function
    def train_step(self, x: tf.Tensor) -> Tuple[tf.Tensor, tf.Tensor, tf.Tensor]:
        """
        ...
        with tf.GradientTape() as tape:
            ...
            total_loss, nll_loss, kde_loss = self.compute_loss(x)
    ...
    def train(self, train_data: tf.Tensor, val_data: Optional[tf.Tensor] = None,
              epochs: int = 100, batch_size: int = 512, validation_freq: int = 10,
              verbose: bool = True) -> Dict[str, list]:
        """
        Train the KDE-informed continuous flow
        ...
        print(f"🚀 Starting KDE-informed continuous flow training")
        ...
        for epoch in range(epochs):
            ...
            if val_data is not None and (epoch + 1) % validation_freq == 0:
                val_loss, val_nll, val_kde = self.compute_loss(val_data)
...
def create_toy_dataset(n_samples: int = 10000, n_dim: int = 6, seed: int = 42) -> Tuple[np.ndarray, gaussian_kde]:
    """
    Create a toy dataset for testing KDE-informed continuous flow training
...
def create_astrophysical_toy_dataset(n_samples: int = 10000, seed: int = 42) -> Tuple[np.ndarray, gaussian_kde]:
    """
    Create an astrophysically-motivated toy dataset for continuous flows
...
def plot_continuous_training_results(history: Dict[str, list], save_path: Optional[str] = None):
    """
    Plot continuous flow training results
...
def compare_continuous_flow_vs_kde_samples(cnf_flow: CNFNormalizingFlow, kde_model: gaussian_kde,
                                          n_samples: int = 5000, save_path: Optional[str] = None):
    """
    Compare samples from continuous flow and KDE models
    
    Parameters:
    -----------
    cnf_flow : CNFNormalizingFlow
        Trained continuous flow model
    kde_model : gaussian_kde
        KDE model
    n_samples : int
        Number of samples to generate
    ...
    """
    print(f"🔍 Comparing continuous flow vs KDE samples ({n_samples} samples each)")
    
    # Generate samples
    cnf_samples = cnf_flow.sample(n_samples, seed=42).numpy()
    ...
    for i in range(3):
        ...
        axes[0, i].hist(cnf_samples[:, i], bins=50, alpha=0.7, label='CNF', density=True)
        axes[0, i].set_title(f'Continuous Flow - Dimension {i+1}')
    ...
    # Print statistics
    print("\n📈 Sample Statistics:")
    for i in range(min(6, cnf_samples.shape[1])):
        cnf_mean, cnf_std = cnf_samples[:, i].mean(), cnf_samples[:, i].std()
        kde_mean, kde_std = kde_samples[:, i].mean(), kde_samples[:, i].std()
        print(f"   Dim {i+1}: CNF({cnf_mean:.3f}±{cnf_std:.3f}) vs KDE({kde_mean:.3f}±{kde_std:.3f})")


def main():
    """Main function to run KDE-informed continuous flow training"""
    parser = argparse.ArgumentParser(description='KDE-Informed Continuous Flow Training')
    ...
    parser.add_argument('--output_dir', type=str, default='./kde_continuous_flow_output', help='Output directory')
    ...
    print("🌟 KDE-Informed Continuous Flow Training")
    print("=" * 60)
    ...
    # Create continuous flow model
    print("🏗️ Creating continuous flow model...")
    cnf_flow = CNFNormalizingFlow(
        input_dim=n_dim,
        ...
        name='kde_informed_continuous_flow'
    )
    
    # Create KDE-informed trainer
    print("🎯 Creating KDE-informed continuous flow trainer...")
    trainer = KDEInformedCNFTrainer(
        flow=cnf_flow,
        kde_model=kde_model,
    ...
    # Train the model
    print("🏋️ Training KDE-informed continuous flow...")
    ...
    # Save model
    model_path = output_dir / "kde_informed_continuous_flow.npz"
    cnf_flow.save(str(model_path))
    
    # Plot training results
    plot_path = output_dir / "continuous_training_results.png"
    plot_continuous_training_results(history, str(plot_path))
    
    # Compare continuous flow vs KDE samples
    comparison_path = output_dir / "continuous_flow_vs_kde_comparison.png"
    compare_continuous_flow_vs_kde_samples(cnf_flow, kde_model, n_samples=5000, save_path=str(comparison_path))
    
    # Save training history
    history_path = output_dir / "continuous_training_history.npz"
    np.savez(history_path, **history)
    
    print(f"\n✅ KDE-informed continuous flow training completed!")
