...
# 🧠 Smart Continuous Flow Batch Submission (50 Array Limit)
...
echo "🧠 SMART CONTINUOUS FLOW BATCH SUBMISSION (50 ARRAY LIMIT)"
echo "=========================================================="
...
show_usage() {
    ...
    echo ""
    echo "PROCESS:"
    echo "  1. Filters completed continuous flow particles"
...
if [[ "$DRY_RUN" != "true" ]]; then
    if [[ ! -f "particle_list_continuous_incomplete.txt" || ! -s "particle_list_continuous_incomplete.txt" ]]; then
        echo ""
        echo "🎉 ALL CONTINUOUS FLOW PARTICLES COMPLETED!"
        echo "==========================================="
    ...
    INCOMPLETE_COUNT=$(grep "Would contain" <<< "$(./filter_completed_continuous.sh --dry-run 2>/dev/null)" | grep -o '[0-9]\+' || echo "460")
    echo ""
...
echo ""
echo "📋 CONTINUOUS FLOW BATCH SUBMISSION PLAN"
echo "========================================"
...
if [[ "$DRY_RUN" == "true" ]]; then
    ...
    for (( batch=1; batch<=MAX_ARRAYS; batch++ )); do
        ...
        if [[ $start_line -le $INCOMPLETE_COUNT ]]; then
            ...
            echo "  sbatch --job-name=continuous_batch_$batch --time=12:00:00 --mem=128GB --gres=gpu:1 --partition=$PARTITION continuous_batch_job.sh $start_line $end_line"
        fi
    done
    ...
    echo "🚀 STEP 3: Submitting continuous flow batches"
    echo "============================================="
    ...
    for (( batch=1; batch<=MAX_ARRAYS; batch++ )); do
        ...
        if [[ $start_line -le $INCOMPLETE_COUNT ]]; then
            ...
            JOB_ID=$(sbatch \
                --job-name="continuous_batch_$batch" \
                --time=12:00:00 \
                --mem=128GB \
                --gres=gpu:1 \
                --partition=$PARTITION \
                --output="logs/continuous_batch_%j.out" \
                --error="logs/continuous_batch_%j.err" \
                continuous_batch_job.sh $start_line $end_line | grep -o '[0-9]\+')
    ...
    echo ""
    echo "🎉 CONTINUOUS FLOW BATCH SUBMISSION COMPLETED"
    echo "============================================="
