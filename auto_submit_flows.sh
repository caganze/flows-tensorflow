...
WAIT_TIME=30      # Seconds to wait between checks
...
find_h5_files_and_particles() {
    ...
    local h5_files=($(find /oak/stanford/orgs/kipac/users/caganze/milkyway-eden-mocks/ -name "eden_scaled_Halo*_particles.h5" -type f 2>/dev/null))
...
check_completed_particles() {
    ...
    if [[ -d "$OUTPUT_BASE_DIR/trained_flows" ]]; then
        for model_dir in "$OUTPUT_BASE_DIR/trained_flows"/model_pid*/; do
            if [[ -d "$model_dir" ]]; then
                local pid=$(basename "$model_dir" | sed 's/model_pid//')
...
main_submission_loop() {
    ...
    echo "║  ⏰ Queue monitoring: Every $WAIT_TIME seconds                                ║"
    echo "╚══════════════════════════════════════════════════════════════════════════════╝"
    echo
    ...
    for ((i=0; i<$num_needed; i+=pids_per_batch)); do
        ...
        while [[ $current_jobs -ge 50 ]]; do  # Conservative limit
            echo "⏳ Queue full ($current_jobs jobs), waiting $WAIT_TIME seconds..."
            sleep $WAIT_TIME
            current_jobs=$(check_queue_status)
        done
