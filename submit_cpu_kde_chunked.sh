...
# 💻 CPU Parallel Chunked Submission - KDE
# For large particle lists - submits in manageable chunks for KDE CPU processing
...
echo "💻 CPU CHUNKED SUBMISSION - KDE"
echo "==============================="
echo "🚀 Submit large particle lists in manageable chunks for KDE training"
echo ""
...
show_usage() {
    ...
    echo "  --time HOURS        Time limit (default: 6:00:00)"
    echo "  --memory SIZE       Memory per task (default: 32GB)"
    echo "  --cpus N            CPUs per task (default: 4)"
    echo "  --dry-run           Show what would be submitted"
    echo "  --help              Show this help"
    echo ""
    echo "EXAMPLES:"
    echo "  $0                              # Submit in 50-particle chunks"
    echo "  $0 --chunk-size 100             # Larger chunks"
    echo "  $0 --start-chunk 3              # Start from 3rd chunk"
    echo "  $0 --particle-list particle_list_kde_incomplete.txt  # Use filtered list"
...
# Check prerequisites
if [[ ! -f "kde_cpu_job.sh" ]]; then
    echo "❌ kde_cpu_job.sh not found"
    exit 1
fi
...
echo ""
echo "📋 KDE CHUNKED SUBMISSION PLAN"
echo "==============================="
...
for (( chunk=START_CHUNK; chunk<=TOTAL_CHUNKS; chunk++ )); do
    ...
    echo "📦 KDE Chunk $chunk: particles $start_particle-$end_particle ($chunk_size_actual particles)"
    
    if [[ "$DRY_RUN" == "true" ]]; then
        echo "  sbatch --array=$start_particle-$end_particle%$CONCURRENT --partition=$PARTITION --time=$TIME_LIMIT --mem=$MEMORY --cpus-per-task=$CPUS kde_cpu_job.sh"
    else
