#!/bin/bash
#SBATCH --job-name="regular_batch"
#SBATCH --partition=owners
#SBATCH --time=12:00:00
#SBATCH --mem=128GB
#SBATCH --gres=gpu:1
#SBATCH --output=logs/regular_batch_%j.out
#SBATCH --error=logs/regular_batch_%j.err

set -e

echo "🚀 REGULAR FLOW BATCH JOB - Job ID ${SLURM_JOB_ID:-test}"
echo "🔧 Mode: Batch Regular Flow Processing (For Loop)"
echo "Started: $(date)"
...
echo "📋 Processing lines $START_LINE to $END_LINE from particle_list_incomplete.txt"
...
PARTICLE_LIST_FILE="particle_list_incomplete.txt"
...
for (( line_num=START_LINE; line_num<=END_LINE; line_num++ )); do
    ...
    # Output directories - use consistent tfp_output directory (regular flows)
    ...
    if [[ -f "$MODEL_DIR/model_pid${SELECTED_PID}.npz" ]] && [[ -f "$SAMPLES_DIR/model_pid${SELECTED_PID}_samples.npz" || -f "$SAMPLES_DIR/model_pid${SELECTED_PID}_samples.h5" ]]; then
        ...
        echo "$(date) SUCCESS halo:$HALO_ID pid:$SELECTED_PID already_completed_batch" >> success_logs/brute_force_success.log
    ...
    if [[ $OBJECT_COUNT -gt 100000 ]]; then
        ...
        HIDDEN_UNITS=512
        LEARNING_RATE=2e-4
        echo "  🧠 Large particle: epochs=60, layers=4, units=512, lr=2e-4"
        ...
        HIDDEN_UNITS=384
        LEARNING_RATE=3e-4
        echo "  🧠 Medium-large: epochs=45, layers=3, units=384, lr=3e-4"
        ...
        HIDDEN_UNITS=256
        LEARNING_RATE=5e-4
        echo "  🧠 Small particle: epochs=35, layers=3, units=256, lr=5e-4"
    else
        EPOCHS=40
        BATCH_SIZE=512
        N_LAYERS=3
        HIDDEN_UNITS=320
        LEARNING_RATE=4e-4
        echo "  🧠 Medium particle: epochs=40, layers=3, units=320, lr=4e-4"
    fi
    
    # Run regular flow training
    echo "  🚀 Training regular flow..."
    ...
    python train_tfp_flows.py \
        ...
        --generate-samples || TRAIN_EXIT=$?
    
    if [[ $TRAIN_EXIT -eq 0 ]]; then
        # Check success
        if [[ -f "$MODEL_DIR/model_pid${SELECTED_PID}.npz" ]] && [[ -f "$SAMPLES_DIR/model_pid${SELECTED_PID}_samples.npz" || -f "$SAMPLES_DIR/model_pid${SELECTED_PID}_samples.h5" ]]; then
            echo "  ✅ SUCCESS: Regular Flow $HALO_ID PID $SELECTED_PID"
            echo "$(date) SUCCESS halo:$HALO_ID pid:$SELECTED_PID training_completed_batch" >> success_logs/brute_force_success.log
            SUCCEEDED=$((SUCCEEDED + 1))
        else
            echo "  ❌ FAILED: Missing output files"
            echo "$(date) FAILED halo:$HALO_ID pid:$SELECTED_PID missing_outputs_batch" >> failed_jobs/brute_force_failures.log
            FAILED=$((FAILED + 1))
        fi
    else
        echo "  ❌ FAILED: Training exit code $TRAIN_EXIT"
        echo "$(date) FAILED halo:$HALO_ID pid:$SELECTED_PID training_failed_${TRAIN_EXIT}_batch" >> failed_jobs/brute_force_failures.log
        FAILED=$((FAILED + 1))
    fi
