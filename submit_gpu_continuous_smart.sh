...
# 🧠 Smart GPU Submission - Continuous Flows
# Automatically filters completed particles and submits only incomplete ones for continuous flow training
...
echo "🧠 SMART GPU SUBMISSION - CONTINUOUS FLOWS"
echo "==========================================="
echo "🔍 Filters completed particles before submission"
echo "🚀 Submits only incomplete particles for continuous flow training"
...
show_usage() {
    ...
    echo ""
    echo "PROCESS:"
    echo "  1. Scans for completed continuous flow models (.npz files)"
    echo "  2. Creates filtered list of incomplete particles"
    echo "  3. Submits in manageable chunks to avoid QOS limits"
    echo "  4. Uses GPU partition for TensorFlow Probability flows"
    echo ""
}
...
echo ""
echo "🔍 STEP 1: Filtering completed continuous flow particles"
echo "======================================================"
...
if ! ./filter_completed_continuous.sh --input "$PARTICLE_LIST_FILE" $FILTER_ARGS; then
...
if [[ "$DRY_RUN" != "true" ]]; then
    if [[ ! -f "particle_list_continuous_incomplete.txt" || ! -s "particle_list_continuous_incomplete.txt" ]]; then
        echo ""
        echo "🎉 ALL CONTINUOUS FLOWS COMPLETED!"
        echo "=================================="
        echo "✅ No incomplete particles found - all continuous flow work is done!"
        exit 0
    fi
    
    INCOMPLETE_COUNT=$(wc -l < particle_list_continuous_incomplete.txt)
    echo ""
    echo "📊 Found $INCOMPLETE_COUNT incomplete continuous flow particles to process"
fi

# Step 2: Submit in chunks
echo ""
echo "🚀 STEP 2: Submitting continuous flow chunks"
echo "============================================"

SUBMIT_ARGS="--chunk-size $CHUNK_SIZE --concurrent $CONCURRENT --partition $PARTITION --particle-list particle_list_continuous_incomplete.txt"

if [[ "$DRY_RUN" == "true" ]]; then
    SUBMIT_ARGS="$SUBMIT_ARGS --dry-run"
    echo "🧪 DRY RUN MODE - Preview of continuous flow submission:"
    echo ""
fi

if ! ./submit_gpu_continuous_chunked.sh $SUBMIT_ARGS; then
    echo "❌ Failed to submit continuous flow chunks"
    exit 1
fi

echo ""
echo "✅ SMART GPU CONTINUOUS FLOW SUBMISSION COMPLETED"
echo "================================================="
...
if [[ "$DRY_RUN" == "true" ]]; then
    ...
    echo "🎯 Only incomplete continuous flow particles were submitted"
    echo "🔄 Jobs should start running soon"
    echo "📁 Output directory: /oak/stanford/orgs/kipac/users/caganze/flows-tensorflow/tfp_output_conditional/"
    echo ""
