#!/usr/bin/env python3
"""
KDE-Informed Conditional Continuous Flow Training Implementation

This script implements KDE-informed training for conditional continuous normalizing flows (CNF) 
that condition on stellar mass. The approach combines KDE and conditional continuous flow training 
through regularization in the loss function.

Key features:
1. Conditional KDE density evaluation for continuous flow training batches
2. Combined loss function: L = L_NLL + λ * L_KDE
3. MSE between conditional continuous flow and KDE log probabilities as regularization
4. Mass-conditioned Neural ODE-based continuous flow architecture
5. Toy dataset generation with mass-phase space correlations
6. Flexible hyperparameter tuning (λ, learning rates, integration parameters, etc.)

ARCHITECTURE: Conditional Continuous Flow
- Learns p(x,y,z,vx,vy,vz|m) - phase space conditioned on mass
- Uses Neural ODEs with mass conditioning
- KDE models created for different mass bins
- MSE loss between conditional CNF and conditional KDE log probabilities

Usage:
    python kde_informed_continuous_flow_conditional_training.py --toy_dataset --epochs 50 --lambda_kde 0.1
"""
...
try:
    from cnf_flows_solution import ConditionalCNFNormalizingFlow, ConditionalCNFFlowTrainer
except ImportError:
    print("Warning: Could not import conditional CNF flow classes. Using simplified versions.")
    
    # Simplified fallback classes
    class ConditionalNeuralODE(tf.keras.Model):
        """Simplified Conditional Neural ODE for continuous flows"""
        
        def __init__(self, input_dim: int, condition_dim: int, hidden_units: List[int] = [64, 64], 
            ...
            # Build network layers with conditioning
            self.layers_list = []
        ...
        def call(self, x, conditions, t=None):
            """Forward pass through Conditional Neural ODE"""
            # Concatenate input with conditions
            h = tf.concat([x, conditions], axis=-1)
    ...
    class ConditionalCNFNormalizingFlow:
        """Simplified Conditional Continuous Normalizing Flow"""
        
        def __init__(self, input_dim: int, condition_dim: int, hidden_units: List[int] = [64, 64], 
                     activation: str = 'tanh', integration_time: float = 1.0,
                     num_integration_steps: int = 10, name: str = 'conditional_cnf_flow'):
            self.input_dim = input_dim
            self.condition_dim = condition_dim
            self.hidden_units = hidden_units
            self.activation = activation
        ...
        def ode_func(self, t, x, conditions):
            """ODE function for integration with conditioning"""
            return self.neural_ode(x, conditions, t)
        
        def forward_transform(self, x, conditions):
            """Forward transform: x -> z with conditioning"""
            # Simple forward pass (in practice, would use ODE solver)
            return self.neural_ode(x, conditions, tf.constant(0.0))
        
        def inverse_transform(self, z, conditions):
            """Inverse transform: z -> x with conditioning"""
            # Simple inverse pass (in practice, would use ODE solver)
            return self.neural_ode(z, conditions, tf.constant(0.0))
        ...
        def log_prob(self, x, conditions):
            ...
            z = self.forward_transform(x, conditions)
            base_log_prob = self.base_dist.log_prob(z)
            # Add log determinant of Jacobian (simplified)
            log_det_jac = tf.reduce_sum(tf.ones_like(x), axis=-1)  # Placeholder
    ...
    class ConditionalCNFFlowTrainer:
        """Simplified Conditional CNF trainer"""
        ...
        def train(self, train_data, train_conditions, val_data=None, val_conditions=None, epochs=100, batch_size=512, validation_freq=10, verbose=True):
            """Train the flow"""
...
class ConditionalKDEInformedCNFTrainer:
    """
    KDE-Informed Conditional Continuous Flow Trainer
    
    This trainer uses conditional KDE density estimates to guide conditional continuous flow training.
    The loss function combines negative log-likelihood with MSE between conditional continuous flow and KDE
    log probabilities: L = L_NLL + λ * L_KDE
    """
    
    def __init__(self, flow: ConditionalCNFNormalizingFlow, kde_models: Dict[float, gaussian_kde],
                 learning_rate: float = 1e-3, lambda_kde: float = 0.1,
                 weight_decay: float = 0.0, noise_std: float = 0.0,
                 mass_bins: int = 10):
        """
        Initialize KDE-informed conditional continuous flow trainer
        
        Parameters:
        -----------
        flow : ConditionalCNFNormalizingFlow
            The conditional continuous flow model to train
    ...
    def compute_mse_log_probs(self, cnf_log_probs: tf.Tensor, kde_log_probs: tf.Tensor) -> tf.Tensor:
        """
        Compute MSE between conditional continuous flow and KDE log probabilities
        
        MSE = E[(log P_CNF - log P_KDE)^2]
        
        Parameters:
        -----------
        cnf_log_probs : tf.Tensor
            Log probabilities from conditional continuous flow
        kde_log_probs : tf.Tensor
        ...
        """
        mse_loss = tf.reduce_mean(tf.square(cnf_log_probs - kde_log_probs))
        return mse_loss
    ...
    def compute_loss(self, x: tf.Tensor, conditions: tf.Tensor) -> Tuple[tf.Tensor, tf.Tensor, tf.Tensor]:
        ...
        Where L_KDE is the MSE between conditional continuous flow and KDE log probabilities:
        L_KDE = E[(log P_CNF - log P_KDE)^2]
        ...
        cnf_log_probs = self.flow.log_prob(x, conditions)
        
        # Clip log probabilities to prevent extreme values
        cnf_log_probs = tf.clip_by_value(cnf_log_probs, -50.0, 50.0)
        
        nll_loss = -tf.reduce_mean(cnf_log_probs)
        
        # KDE regularization loss (MSE between log probabilities)
        kde_log_probs = self.evaluate_conditional_kde_density(x, conditions)
        kde_loss = self.compute_mse_log_probs(cnf_log_probs, kde_log_probs)
    ...
    def train(self, train_data: tf.Tensor, train_conditions: tf.Tensor,
        ...
        """
        Train the KDE-informed conditional continuous flow
        ...
        print(f"🚀 Starting KDE-informed conditional continuous flow training")
        print(f"   Epochs: {epochs}, Batch size: {batch_size}")
        print(f"   λ (KDE weight): {self.lambda_kde}")
        print(f"   Learning rate: {self.optimizer.learning_rate.numpy():.2e}")
        print(f"   Loss: L = L_NLL + λ * MSE(log P_CNF, log P_KDE)")
        print(f"   Integration time: {self.flow.integration_time}")
        print(f"   Integration steps: {self.flow.num_integration_steps}")
...
def plot_conditional_continuous_training_results(history: Dict[str, list], save_path: Optional[str] = None):
    """
    Plot conditional continuous flow training results
...
def compare_conditional_continuous_flow_vs_kde_samples(cnf_flow: ConditionalCNFNormalizingFlow, kde_models: Dict[float, gaussian_kde],
                                                      mass_conditions: np.ndarray, n_samples: int = 1000, 
                                                      save_path: Optional[str] = None):
    """
    Compare samples from conditional continuous flow and KDE models
    
    Parameters:
    -----------
    cnf_flow : ConditionalCNFNormalizingFlow
        Trained conditional continuous flow model
    kde_models : Dict[float, gaussian_kde]
    ...
    """
    print(f"🔍 Comparing conditional continuous flow vs KDE samples")
    ...
    for i, mass_val in enumerate(mass_bins[:n_mass_bins]):
        # Generate conditional continuous flow samples
        mass_condition = tf.constant([[mass_val]], dtype=tf.float32)
        cnf_samples = cnf_flow.sample(n_samples, mass_condition, seed=42).numpy()
        ...
        axes[0, i].hist(cnf_samples[:, 0], bins=30, alpha=0.7, label='CNF', density=True)
        axes[0, i].set_title(f'Conditional CNF - Mass {mass_val:.2f} M☉')
...
def main():
    """Main function to run KDE-informed conditional continuous flow training"""
    parser = argparse.ArgumentParser(description='KDE-Informed Conditional Continuous Flow Training')
    ...
    parser.add_argument('--hidden_units', type=int, nargs='+', default=[64, 64], help='Hidden units per layer')
    parser.add_argument('--activation', type=str, default='tanh', help='Activation function')
    parser.add_argument('--integration_time', type=float, default=1.0, help='Integration time')
    parser.add_argument('--num_integration_steps', type=int, default=10, help='Number of integration steps')
    parser.add_argument('--mass_bins', type=int, default=8, help='Number of mass bins for KDE')
    parser.add_argument('--output_dir', type=str, default='./kde_conditional_continuous_flow_output', help='Output directory')
    ...
    print("🌟 KDE-Informed Conditional Continuous Flow Training")
    print("=" * 60)
    ...
    print(f"  Integration time: {args.integration_time}")
    print(f"  Integration steps: {args.num_integration_steps}")
    ...
    # Create conditional continuous flow model
    print("🏗️ Creating conditional continuous flow model...")
    cnf_flow = ConditionalCNFNormalizingFlow(
        input_dim=n_dim,
        ...
        name='kde_informed_conditional_continuous_flow'
    )
    
    # Create KDE-informed trainer
    print("🎯 Creating KDE-informed conditional continuous flow trainer...")
    trainer = ConditionalKDEInformedCNFTrainer(
        flow=cnf_flow,
    ...
    # Train the model
    print("🏋️ Training KDE-informed conditional continuous flow...")
    ...
    # Save model
    model_path = output_dir / "kde_informed_conditional_continuous_flow.npz"
    cnf_flow.save(str(model_path))
    
    # Plot training results
    plot_path = output_dir / "conditional_continuous_training_results.png"
    plot_conditional_continuous_training_results(history, str(plot_path))
    
    # Compare conditional continuous flow vs KDE samples
    comparison_path = output_dir / "conditional_continuous_flow_vs_kde_comparison.png"
    compare_conditional_continuous_flow_vs_kde_samples(cnf_flow, kde_models, masses, n_samples=1000, save_path=str(comparison_path))
    
    # Save training history
    history_path = output_dir / "conditional_continuous_training_history.npz"
    np.savez(history_path, **history)
    
    print(f"\n✅ KDE-informed conditional continuous flow training completed!")
    ...
    print(f"   - Increase λ (--lambda_kde) to make continuous flow follow KDE more closely")
    print(f"   - Decrease λ to let continuous flow learn more from raw data")
    print(f"   - Adjust --integration_time and --num_integration_steps for ODE solver")
