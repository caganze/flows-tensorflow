#!/bin/bash
#SBATCH --job-name="priority_12h_halos"
#SBATCH --partition=owners
#SBATCH --time=12:00:00
#SBATCH --mem=128GB
#SBATCH --gres=gpu:4
#SBATCH --output=logs/priority_12h_%A_%a.out
#SBATCH --error=logs/priority_12h_%A_%a.err
#SBATCH --array=1-8%8

# 🚀 PRIORITY 12-HOUR GPU JOB FOR CRITICAL HALOS
# Target: symphony halos 239, 718, 270, 925 + eden halos 239, 718, 270, 925
# Focus: Complete ALL particles in these halos within 12 hours

set -e

echo "🚀 PRIORITY 12-HOUR HALO COMPLETION JOB"
echo "========================================"
...
# Get list of particles in this halo
...
try:
    with h5py.File('$SELECTED_H5', 'r') as f:
        if 'PartType1' in f and 'ParticleIDs' in f['PartType1']:
            ...
            print(' '.join(map(str, pids[:50])))  # Limit to first 50 for 12h constraint
        else:
            print('')
...
echo "📊 Found ${#PARTICLES[@]} particles to process"
...
# Process each particle with optimized parameters for 12-hour completion
COMPLETED=0
FAILED=0
START_TIME=$(date +%s)
...
")
    ...
    # Ultra-fast parameters for 12-hour completion
    if [[ $OBJECT_COUNT -gt 100000 ]]; then
        # Very large particles - minimal epochs for speed
        EPOCHS=25
        BATCH_SIZE=1024
        N_LAYERS=3
        HIDDEN_UNITS=256
        LEARNING_RATE=1e-3
        echo "🚀 Ultra-fast Large (>100k): epochs=25, layers=3, units=256, lr=1e-3"
        ...
        HIDDEN_UNITS=320
        LEARNING_RATE=8e-4
        echo "🚀 Ultra-fast Medium-Large (50k-100k): epochs=30, layers=3, units=320, lr=8e-4"
        ...
        HIDDEN_UNITS=128
        LEARNING_RATE=1e-3
        echo "🚀 Ultra-fast Small (<5k): epochs=20, layers=2, units=128, lr=1e-3"
    else
        # Medium particles
        EPOCHS=25
        BATCH_SIZE=512
        ...
        HIDDEN_UNITS=256
        LEARNING_RATE=8e-4
        echo "🚀 Ultra-fast Medium (5k-50k): epochs=25, layers=3, units=256, lr=8e-4"
    fi
    
    # Train with timeout for time management
    timeout 1800 python train_tfp_flows.py \
        ...
        --generate-samples \
        --use_kroupa_imf \
        --model_name "model_pid${PID}" 2>&1 | tee "logs/priority_${SUITE}_halo${HALO_ID}_pid${PID}.log"
    ...
    if [[ $TRAIN_EXIT -eq 0 ]]; then
        ...
        ((COMPLETED++))
        echo "PID_${PID}" >> "success_logs/${SUITE}_halo${HALO_ID}_success.txt"
    else
        echo "❌ PID $PID failed (exit code: $TRAIN_EXIT)"
        ((FAILED++))
        echo "PID_${PID}_EXIT_${TRAIN_EXIT}" >> "failed_jobs/${SUITE}_halo${HALO_ID}_failed.txt"
    fi
...
echo ""
echo "🏁 PRIORITY JOB SUMMARY for $SUITE Halo$HALO_ID"
echo "=============================================="
